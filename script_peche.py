...
def get_wind_dir(u, v):
    """Convertit les vecteurs U/V en direction cardinale"""
...
def get_data(name, b):
    print(f"📡 Scan : {name}...")
    res = {'sst': 21.5, 'vhm0': 1.1, 'wind_speed': 15, 'wind_dir': 'N'}
    ...
    try:
        ...
        # 3. VENT (Extraction U et V à 10m)
        ...
        res['wind_speed'] = round(math.sqrt(u**2 + v**2) * 3.6, 1) # Conversion km/h
...
def main():
    ...
    report += f"📅 <i>{now.strftime('%d/%m/%Y à %H:%M')} GMT</i>\n"
    report += f"───────────────────\n\n"
    ...
    for name, config in ZONES.items():
        ...
        report += f"🌡️ Mer: <b>{data['sst']}°C</b>\n"
        report += f"🌊 Houle: <b>{data['vhm0']}m</b>\n"
        report += f"🌬️ Vent: <b>{data['wind_speed']} km/h</b> ({data['wind_dir']})\n"
        report += f"🎣 {target}\n\n"
        
        web_json.append({**{"zone": name, "alert": alert}, **data})
